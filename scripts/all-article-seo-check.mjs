...
const EXPECTED = {
  ...
  prokrastynatsiia:{ready:['yak-pochaty','tysk-na-sebe'],pendingTitles:['Увага та концентрація','Як змінити себе']}
};
...
assert.equal(new Set(allReady).size,9,'ready hub slugs must be unique'); assert.equal(pendingCount,3,'exactly three future subblocks should be marked pending');
...
const pressureResponse=new MockResponse(); handlePressure({query:{audit:'1'}},pressureResponse); assert.equal(pressureResponse.statusCode,200,'pressure audit must render'); const pressureAudit=JSON.parse(pressureResponse.body); assert.equal(pressureAudit.count,15,'Тиск на себе must contain 15 ready articles');
const nextFive=new Set(['postiinno-treba-buty-produktyvnym','znetsiniuiu-vlasni-rezultaty','samootsinka-zalezhyt-vid-produktyvnosti','sorom-cherez-nevykonani-spravy','karaiu-sebe-pislia-zryvu']);
const checked=pressureAudit.articles.filter(a=>nextFive.has(a.slug)); assert.equal(checked.length,5,'all pressure articles 11–15 must be present');
console.log('Pressure 11–15 audit:',checked.map(({slug,words,minutes})=>`${slug}=${words}w/${minutes}m`).join(', '));
