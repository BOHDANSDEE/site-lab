...
import { ENERGY_STRENGTH_ARTICLES_2 } from '../article-data/energy-strength-articles-2.mjs';
import { ENERGY_STRENGTH_ARTICLES_3 } from '../article-data/energy-strength-articles-3.mjs';
...
const energy=[...ENERGY_STRENGTH_ARTICLES,...ENERGY_STRENGTH_ARTICLES_2,...ENERGY_STRENGTH_ARTICLES_3];
assert.equal(energy.length,20);assert.equal(new Set(energy.map(a=>a.slug)).size,20);assert.equal(new Set(energy.map(a=>a.title)).size,20);
const ec=render({category:'energy'});assert.equal(ec.statusCode,200);assert.ok(ec.body.includes(`<link rel="canonical" href="${SITE}/statti/enerhiia-ta-syly/">`));for(const a of energy){assert.ok(ec.body.includes(`/statti/enerhiia-ta-syly/${a.slug}/`));const r=render({category:'energy',slug:a.slug});assert.equal(r.statusCode,200);assert.ok(r.body.includes(`<link rel="canonical" href="${SITE}/statti/enerhiia-ta-syly/${a.slug}/">`));assert.ok(r.body.includes('<h1>'));assert.ok(r.body.includes('application/ld+json'))}
console.log('✅ SEO check passed: 20 Discipline + 20 Energy articles');
