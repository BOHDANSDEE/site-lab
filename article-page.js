(() => {
  ...
  const focus = {
    ...
    'prokrastynatsiia-i-perfektsionizm':['Перфекціонізм підвищує ціну першої чернетки й робить початок небезпечним для самооцінки.','Створіть версію, яку ніхто не побачить, і забороніть редагування перші десять хвилин.'],
  ...
  const configs = {
    'Лінь': {
      ...
      steps:['Опишіть проблему без ярлика','Перевірте сон, їжу та доступну енергію','Зменшіть дію до 2–10 хвилин','Після кроку запишіть, що допомогло'],
